...
sample_sim <- function(greaterThanPatients, ntrees, training_datasets, rho=0.4){
  ...
  if(nrow(k.bin.rf)>=2){
    ...
    imp_preds = predict(implicitBinRFmodel, exv_df,type = 'prob')[,2]
    base_preds = predict(baseRFmodel, exv_df,type = 'prob')[,2]
    exv_y_numeric = ifelse(exv_y=='Symptomatic',1,0)
    
    # res_imp <- evalmod(scores = imp_preds, labels = exv_y_numeric)
    return(data.frame(implicitRF=imp_preds, 
                baseRF = base_preds, 
                y =exv_y_numeric))
  }
...
filter1 = mclapply(1:nsim, function(x) sample_sim(greaterThanPatients=1, ntrees = ntrees, training_datasets=training_datasets))
filter1

impRf1 = do.call(cbind,lapply(1:1filter1[,'implicitRF'] )  
...
write.csv(final_results,'~/Dropbox/Pathway-Pathway Interaction/results/HRVsignal_degradation.csv')
save(final_results, full_results,file= '~/Dropbox/Pathway-Pathway Interaction/results/interaction_res.RData')
